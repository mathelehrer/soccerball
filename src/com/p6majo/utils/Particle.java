...
import static com.p6majo.linalg.Vector.map;
...
public class Particle implements ObjectIn3DSpace {
    ...
    private Vector randomPosition(Vector low, Vector high){
        double x = map(Math.random(),0,1,low.getX(),high.getX());
        double y = map(Math.random(),0,1,low.getY(),high.getY());
        double z = map(Math.random(),0,1,low.getZ(),high.getZ());
