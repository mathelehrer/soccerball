...
import com.p6majo.povrayanimation.objects.Cylinder;
import com.p6majo.povrayanimation.objects.OrientedPrism;
import com.p6majo.povrayanimation.objects.PovrayObject;
import com.p6majo.povrayanimation.objects.Sphere;
...
public class PovrayViewSoccerball extends PovrayAnimation {
    ...
    private List<ModelSoccerBall.Face> faces;
    ...
    private List<PovrayObject> corners(List<Particle> particles){
        ...
        for (Particle particle : particles) {
            objects.add(new Sphere(particle.getPosition().toVector3D(),particle.getRadius(),new SphereAppearance(Color.RED.darker())));
        }
        return objects;
    }
        ...
        for (Particle particle : particles) {
            ...
            for (Particle particle1 : particles) {
                if (!neighbours.contains(particle1) && !particle1.equals(nnParticle)){
                    ...
                    if (s<0)
                        color =new Color(255,255+s,255+s);
                    else
                        color =new Color(255-s,255-s,255);
    ...
    private List<PovrayObject> faces(List<Particle> particles){
        ...
        List<PovrayObject> objects = new ArrayList<>();
        int faceCounter=0;
        for (ModelSoccerBall.Face face : faces) {
